...
wrong.tweets <- c(
"RT @ATLBlackStar: GA waitress Emelia Holden is seen on camera being sexually violated by 31 yr old Ryan Cherwinksi. Holden slammed hi… https://t.co/TSBSiSPbnF",
"RT @JohnBrennan: Watching the spectacle of the House “hearing” with Peter Strzok today, I was reminded of the words of Abraham Linco… https://t.co/Khe98Rn6Xe",
"RT @CNN: Former President Obama: “I believe in Nelson Mandela’s vision. I believe in a vision shared by Gandhi and King and… https://t.co/Ru81sjwL2F",
"RT @PETTYMAMII: If my bestfriend have a car..THE PASSENGER SEAT IS MY SEAT &amp; vice versa!! Idgaf if you was riding with her 5 hours… https://t.co/3CQ7FqR4af",
"RT @GirlFromBlupo: Dear Amazon, I bought a toilet seat because I needed one. Necessity, not desire. I do not collect them. I am not a… https://t.co/w1yKIqx6w5",
"RT @charliekirk11: Liberals suddenly stopped caring about kids at the border as soon as that Supreme Court seat opened up",
"RT @RepAdamSchiff: McConnell held a Supreme Court seat open for a year, insisting no Justice be confirmed in an election year without voters having a say. This was a terrible precedent but one he will need to live with. Dem Senators must insist — absolutely no confirmation until after the midterm."
)
...
data.hyundai <- dk.load(file.tweet.hyundai, file.index.hyundai, T)
data.gm <- dk.load(file.tweet.gm, file.index.gm, T)
data.toyota <- dk.load(file.tweet.toyota, file.index.toyota, T)
data.ford <- dk.load(file.tweet.ford, file.index.ford, T)
data.vw <- dk.load(file.tweet.vw, file.index.vw, T)
...
resultsGraphWidth  <- 6 # 9
resultsGraphHeight <- 4 # 5
...
#ggplotRegression(fit2) + xlab("Retweet Ratio") + ylab("Absolute Difference") + labs(title = "")
dev.off()
...
grangertest(Close ~ SA_TB, order = 1, data = data.toyota)
grangertest(Close ~ SA_TB, order = 7, data = data.gm)
...
granger.x$sig <- rowSums(granger.x[2:5] <= 0.05)
granger.y$sig <- rowSums(granger.y[2:5] <= 0.05)

granger.x$sig.tb <- granger.x$SA_TB <= 0.05
granger.x$sig.nb <- granger.x$SA_NB <= 0.05
granger.x$sig.me <- granger.x$SA_ME <= 0.05
granger.x$sig.svm <- granger.x$SA_SVM <= 0.05

fit.x <- lm(sig ~ lag + company + SA_TB + SA_NB + SA_ME + SA_SVM, data = granger.x)
fit.y <- lm(sig ~ lag + company + SA_TB + SA_NB + SA_ME + SA_SVM, data = granger.y)

summary(fit.x)
summary(fit.y)

group_by(granger.x, lag) %>% summarize(sum(sig.tb)/n(),sum(sig.nb)/n(),sum(sig.me)/n(),sum(sig.svm)/n())

group_by(granger.x, lag) %>% summarize(sum(sig))
group_by(granger.y, lag) %>% summarize(sum(sig))

granger <- granger.x %>%		
	left_join(granger.y, by = c("lag" = "lag", "company" = "company"))

# significant per lag/company/classifier
granger$sig.t <- rowSums(granger[2:5] <= 0.05)
granger$sig.n <- rowSums(granger[8:11] <= 0.05)
granger$sig <- granger$sig.t + granger$sig.n
...
granger.base <- aggregate(significant ~ type + company + rt.omitted + lag, granger.melted, function(x) sum(x == 0))
