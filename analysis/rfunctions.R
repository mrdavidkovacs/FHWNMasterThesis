...
rootPath <- "C:\\Users\\David Kovacs\\Documents\\MasterThesisCode"
...
dk.load <- function(tweetFile, indicesFile, omitRt = F, omitWrongTopics = F) {
	...
	if(omitWrongTopics) {
		print(paste0("count before: ", nrow(tweets_raw)))
		tic("> filtering retweets - wrong tweets")
		tweets_raw <- tweets_raw[!(tweets_raw$text %in% wrong.tweets),]
		toc()
		print(paste0("count after: ", nrow(tweets_raw)))
	}
...
dk.gathertweetsbydate <- function(tweetFile, dates) {
	...
	#tic(paste0("> write ", nrow(filtered), " tweets to file"))
	#write.table(filtered, file = paste(tweetsPath, paste0(tweetFile, '_filtered', '.csv'), sep="/"), row.names=F, na="", col.names=T, quote = T, sep="\t")
	#toc()
}
...
dk.grangerstat <- function(data) {
	...
	#x <- data %>% group_by(company, type, rt.omitted, significant) %>% summarize(count=n())
	#print(dcast(x, company ~ type + rt.omitted + significant))
...
dk.sentimentstats <- function(df) {
	...
	#print(paste0(dk.formatnumber(sum(df$SA_TB > 0) / nrow(df) * 100, 2), ' % TB'))
	#print(paste0(dk.formatnumber(sum(df$SA_NB > 0) / nrow(df) * 100, 2), ' % NB'))
	#print(paste0(dk.formatnumber(sum(df$SA_ME > 0) / nrow(df) * 100, 2), ' % ME'))
	#print(paste0(dk.formatnumber(sum(df$SA_SVM > 0) / nrow(df) * 100, 2), ' % SVM'))
}
