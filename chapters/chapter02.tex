...
\rework{\citet{Bollen2011a}} noted that many researches assumed that the stock market is based on the random walk theory and the \ac{EMH}.
...
\begin{description}
	...
	\item [Stop words]
		...
		Examples for stop words are 'a', 'the' and 'for' for the \rework{English} language
...
\rework{Therefore the order of tokens is irrelevant to build a suitable representation of the text.
...
\rework{In the following each of them is introduced shortly.}
...
\rework{Messages on Twitter are called tweets and are limited in length (140 for tweets before \printdate{2017-11-07}; 280 characters since then, at least for English tweets \citep{Rosen2017}) and due to this fact users have to concentrate on a specific topic precisely.
