...
Preprocessing may include removal of stop words, stemming of words.

% Stop words

Stop words are commonly used words of a language which do not transport any sentiment nor opinion, for example 'a', 'the', 'about', \ldots\ for the english language.

% Stemming

Stemming is a process to unify words to a single style.
For example the words 'go' and 'going' are meaning the same thing but would yield to different features.
Therefore, stemmers transforming different tenses and gerund forms of a word to the general form.
