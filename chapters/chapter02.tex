...
\citeauthor{Bollen2011a} noted that many researches assumed that the stock market is based on the random walk theory and the \gls{EMH}.
First, the \gls{EMH} suggests that the price of a security is reflected by all information available \cite{fama1965behavior, schumaker2009textual}.
Furthermore, the \gls{EMH} can be split into three forms: the Weak, the Semi-Strong and the Strong.

\begin{itemize}
    %\itemsep0em
    \item
        In Weak \gls{EMH} only historical information is incorporated in the current price \cite{schumaker2009textual}.

    \item
        In Semi-Strong \gls{EMH} uses historical and also currently available public information in the current price \cite{schumaker2009textual}.

    \item
        In Strong \gls{EMH} the Semi-Strong model is enhanced with currently available private information such as insider information in the share price \cite{schumaker2009textual}.
\end{itemize}

Therefore, the \gls{EMH} assumes that stock market prices are driven by new information such as news and will less depend on the current price or historical prices.
...
\begin{description}
	\item[Naive Bayes.] 
    The \gls{NB} algorithm calculates the probabilities that a document $d$ belong to a class $c$.
	...
	The \gls{ME} Classification succeeded in a number of language processing applications.
	...
	\gls{SVM} are large-margin classifiers in contrast to the probabilistic Naive Bayes and Maximum Entropy.
...
\begin{description}
	\item [Confusion Matrix]
		...
		If the correct label is \emph{positive} and the algorithm predicted \emph{positive}, than it is a \gls{TP}.
		On the other hand if the algorithm predicted \emph{negative} it is a \gls{FN}.
		The same structure applies if the correct label is \emph{negative}: \gls{TN} if the algorithm predicted \emph{negative} and \gls{FP} if the algorithm predicted \emph{positive}
		\cite{Tripathy2015}.
