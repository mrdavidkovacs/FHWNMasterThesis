...
\begin{description}
  ...
  \item[Comparing Sentiment Time Series with Share Prices]
    ...
    As negative sentiments have the value \emph{-1} and positive sentiments have the value \emph{1} we receive a number which is positive in case more positive than negative tweets have been published on that given day and vice versa. 
    ...
    These steps are repeated as long values are missing
    \cite{Pagolu2016a}. 

    The same procedure has been applied to the sentiment time series.

    %The comparison of both time series is performed using the R software package v3.5.2 \cite{R2008}.
